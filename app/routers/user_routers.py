...
async def get(db:Session=Depends(get_db), Authorization: str = Header(None)):
    try:
        ...
        return UserResponse(code=200, status="ok",message="Success fetch all data", result=_users)
    except:
         return UserResponse(code=401, status="fail",message="Invalid token")
...
async def get_current_user(db:Session=Depends(get_db), Authorization: str = Header(None)):
    try:
        ...
        return UserResponse(code=200, status="ok",message="Success fetch current user", result=_user)
    except:
         return UserResponse(code=401, status="fail",message="Invalid token")
...
async def update_user(user_id, db:Session=Depends(get_db), Authorization: str = Header(None)):
    try:
        ...
        if exist_user == None:
            return UserResponse(code=200, status="ok",message="the user does not exist")
